...
pub fn build_buttons(surf_w: f32, surf_h: f32, sf: f32) -> Vec<Button> {
    ...
    vec![
        Button { text: "Play", top_left: Pos2::new(start_x, start_y), width: btn_w, height: btn_h, slant },
        Button { text: "Settings", top_left: Pos2::new(start_x, start_y + btn_h + gap), width: btn_w, height: btn_h, slant },
        Button { text: "Exit", top_left: Pos2::new(start_x, start_y + (btn_h + gap) * 2.0), width: btn_w, height: btn_h, slant },
    ]
}
...
pub fn build_mode_buttons(surf_w: f32, surf_h: f32, sf: f32) -> Vec<Button> {
    let logical_w = surf_w / sf;
    let logical_h = surf_h / sf;
    ...
    vec![
        ...
        Button { text: "Blitz",  top_left: Pos2::new(start_x, start_y + btn_h + gap),           width: btn_w, height: btn_h, slant },
        Button { text: "Custom", top_left: Pos2::new(start_x, start_y + (btn_h + gap) * 2.0),   width: btn_w, height: btn_h, slant },
    ]
}

pub fn handle_mode_click(mouse_pos: Pos2, buttons: &[Button]) -> ModeAction {
    for button in buttons {
        if button.contains(mouse_pos) {
            return match button.text {
...
pub fn render_mode_select(
    ...
    let buttons = build_mode_buttons(surf_w as f32, surf_h as f32, sf);
